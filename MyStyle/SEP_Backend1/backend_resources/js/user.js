...
//                });
        ...
        submitHandler: function(form) {
            $('#user_msg').html('<div class="info canhide">Changing password...</div>');
            
            $.post(site_url + '/user/user_change_password', $("#reg_usr_pw_chng_form").serialize(), function(msg) {
                //$('#user_msg').html(msg); return;
                if(msg == 1)
                    {
                        
                        $('#user_msg').html('<div class="success canhide">Password successfully changed.</div>');
                        window.location = site_url+"/user/registeredUsers";
                    }
                    
                    else
                        {
                            $('#user_msg').html('<div class="warning">Error changing password. Try again !</div>');
                        }
        ...
        $('#admn_usr_pw_chng_form').validate({
        rules: {
            usr_pw: 'required',
            cnfrm_usr_pw: {
                required: true,
        ...
        submitHandler: function(form) {
            ...
            $.post(site_url + '/admin_users/admn_user_change_password', $("#admn_usr_pw_chng_form").serialize(), function(msg) {
                //$('#user_msg').html(msg); return;
                if(msg == 1)
                    {
   ...
   $(document).on('click','.change_reg_user_status',function(){
        ...
        submitHandler: function(form) {
            ...
            $.post(site_url + '/admin_users/saveUser', $("#admin_usr_add_form").serialize(), function(msg) {
                ...
                $('#custom_alert').html('<div class="success canhide">User successfully added.</div>');
                window.location = site_url+"/admin_users/manage_users";
                }else{
                    $('#custom_alert').html('<div class="warning">Error adding user. Try again !</div>');
                }
            });
        }
    });
                ...
                if(msg ==1)
                    {
                        ...
                        $('#custom_alert').html('<div class="success canhide">User successfully updated.</div>');
                        window.location = site_url+"/admin_users/manage_users";
                    }
                    
                    else
                        {
                            $('#custom_alert').html('<div class="warning">Error updating user. Try again !</div>');
                        }
    ...
    $(document).on('click','.delete_admn_usr',function(){
            ...
            $.post(site_url + '/admin_users/deleteUser','user_id='+user_id,function(msg){
               ...
               // alert(msg);
                if(msg ==1)
                    {
                        //location.reload(); 
                        $('#custom_alert').html('<div class="success canhide">User successfully deleted.</div>');
                        $('#row_'+user_id).hide();
                    }
                    
                    else
                        {
                            $('#custom_alert').html('<div class="warning">Error deleting user. Try again !</div>');
                        }
       ...
       $(document).on('click', '#grant_revoke_privilege .change_user_privilege', function(){
        ...
        else
        {
            ...
            //else
            //{
                $("#alertspan_"+alertspanid).html('<b style=" color: blue;" title="Processing">... </b>');
            //}
            //return;
            $.post(site_url + '/admin_users/change_privilege_save', {user_id: $('#admn_usr_slct').val(), privilege_id: $(this).val(), grant_privilege: $(this).is(':checked')}, function(msg) {
                ...
                if(jQuery.trim(msg.split(' ########## ')[1]) == "1")
                    {
                        if(jQuery.trim(msg.split(' ########## ')[0]) == 'true')
                            {
                                //alert('true');
                                $("#alertspan_"+alertspanid).html('<b style=" color: green;" title="Privilege Granted">✓ </b>');
                            }
                            else if(jQuery.trim(msg.split(' ########## ')[0]) == 'false')
                                {
                                    //alert('false');
                                    $("#alertspan_"+alertspanid).html('<b style=" color: red;" title="Privilege Revoked">✓ </b>');
                                }
                        
                    }
                    else if(msg == "0")
                        {
                            $("#alertspan_"+alertspanid).html('<b style=" color: red;" title="Error. Try again.">x </b>');
                        }
			...
			//Start : update new password
				...
				.ajax({
					...
					if(changd_pw_rtn_msg==0){
						...
						$('#cstm_msg').html('<div class="success canhide">Password successfully changed.</div>');
